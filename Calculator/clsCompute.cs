    ...
    class clsCompute
    {
        ...
        public static double calculate(string sum)
        {
            ...
            try
            {
                ...
                if (validateExpression(strtmpExpression))
                {
                            ...
                            if (strtmpExpression[i] == '(')
                            {
                                ...
                                for (int j = i; j < strtmpExpression.Length; j++)
                                {
                                    if (strtmpExpression[j] == ')')
                                    {
                                        ...
                                        if (compute(strtmpBracket, out dbltmpResult))
                                        {
                    ...
                    if (!blnExit)
                        compute(strtmpExpression, out dbltmpResult);
                }
            }
            catch (Exception ex)
            {
        ...
        public static bool validateExpression(string strExpression)
        {
        ...
        #region compute
        private static bool compute(string strExpression, out double dbltmpResult)
        {
                ...
                if (blnReturn)
                {
                    ...
                    if (chOperator.Contains(opMul) || chOperator.Contains(opDiv))
                    {
                        for (int i = 0; i < chOperator.Count; i++)
                        {
                            ...
                            {
                                if (cal2Num(dblNum[i], dblNum[i + 1], chOperator[i], out dbltmpResult))
                                {
                ...
                if (blnReturn)
                {
                    ...
                    if (chOperator.Contains(opPlus) || chOperator.Contains(opMinus))
                    {
                        for (int j = 0; j < chOperator.Count; j++)
                        {
                            ...
                            {
                                if (cal2Num(dblNum[j], dblNum[j + 1], chOperator[j], out dbltmpResult))
                                {
        ...
        #region cal2Num
        private static bool cal2Num(double dblNum1, double dblNum2, char op, out double dblResult)
        {
            dblResult = 0;
            bool blnReturn = true;
